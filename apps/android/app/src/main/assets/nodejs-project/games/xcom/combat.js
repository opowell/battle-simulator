...
export function calcHitChance(shooter, target, board) {
  ...
  let chance  = shooter.attrs.aim - dist * 4 - cover;
  if (shooter.type === 'sniper') chance += dist * 3; // sniper gains accuracy with range
  return Math.max(5, Math.min(95, chance));
}

export function rollHit(hitChance, rng) {
  const roll = Math.floor(rng() * 100) + 1;
  return { hit: roll <= hitChance, crit: roll <= Math.floor(hitChance / 4), roll };
}
