...
import { Sc1Belief } from './belief.js';
...
test('sc1 fog: getVisibleState hides distant enemies and stores a roster', () => {
  const s    = Sc1Game.createInitialState(players(), { fogOfWar: true });
  const view = Sc1Game.getVisibleState(s, 'p1');
  // p1 sees its own units; p2 starts in the opposite corner, well out of VISION=3
  ...
  assert.ok(view.buildings.filter(b => b.ownerId === 'p2').length === 0, 'far p2 buildings hidden');
  assert.ok(s.gameSpecific.startRoster.units.length > 0, 'startRoster populated');
  assert.ok(s.gameSpecific.startRoster.buildings.length > 0, 'startRoster has buildings');
});

test('sc1 fog: sampleWorlds places hidden enemies outside our vision', () => {
  const s     = Sc1Game.createInitialState(players(), { fogOfWar: true });
  const view  = Sc1Game.getVisibleState(s, 'p1');
  const worlds = Sc1Game.sampleWorlds(view, 'p1', 8);
...
test('sc1 fog: sampleWorlds returns [] when fog is off', () => {
  const s = Sc1Game.createInitialState(players(), { fogOfWar: false });
  assert.deepEqual(Sc1Game.sampleWorlds(Sc1Game.getVisibleState(s, 'p1'), 'p1', 4), []);
});

test('sc1 fog: belief pins a sighting then expands within move range', () => {
  const s      = Sc1Game.createInitialState(players(), { fogOfWar: true });
  const roster = s.gameSpecific.startRoster;
  const enemy  = roster.units.find(u => u.ownerId === 'p2');
  const belief = new Sc1Belief('p1', s.board,
    roster.units.filter(u => u.ownerId === 'p2'),
    roster.buildings.filter(b => b.ownerId === 'p2'),
  );

  // Turn 1: one of our units is right next to the enemy unit.
  const near = { x: enemy.position.x - 1, y: enemy.position.y };
  belief.beginTurn({
    ...s, turnNumber: 1,
    units: [
      { id: 'p1-scout', ownerId: 'p1', type: 'marine', position: near, alive: true, hp: 40 },
  ...
  belief.beginTurn({
    ...s, turnNumber: 2,
    units: [{ id: 'p1-scout', ownerId: 'p1', type: 'marine', position: { x: 0, y: 0 }, alive: true, hp: 40 }],
    buildings: [],
  });
...
test('sc1 fog: witnessed building destruction is not resurrected', () => {
  const s      = Sc1Game.createInitialState(players(), { fogOfWar: true });
  const roster = s.gameSpecific.startRoster;
  ...
  // Observe the enemy building right next to us, then dead.
  const near = { x: enemyBldg.position.x - 1, y: enemyBldg.position.y };
  belief.beginTurn({
    ...s, turnNumber: 1,
    units: [{ id: 'p1-scout', ownerId: 'p1', type: 'marine', position: near, alive: true, hp: 40 }],
  ...
  const worlds = belief.sample(
    { ...s, units: [{ id: 'p1-scout', ownerId: 'p1', type: 'marine', position: near, alive: true, hp: 40 }], buildings: [] },
    5, Math.random,
