...
Shipped and tested (`games/chess/analysis.test.js`):
...
(`apps/design/analysis-worker.js`), off both the server and the UI thread.
...
- **`games/chess/stockfish.js` is browser-safe** — its Node imports
...
- **`/lib/*` static route** (`api-server.js` serveLibModule) serves repo ES
  modules to the worker (JS/WASM only, path-escape-guarded). The worker imports
  `../../lib/games/chess/…` (relative, so a mounted base path resolves too).
...
plays — `onActionCommitted` fires solely from `agents/ObscuroAgent.js` (the
...
`games/chess/exactBelief.js`'s `ExactBelief.positions` is `Int8Array(66)[]` —
...
- **Portable as-is (plain ESM, no Node deps):** `agents/obscuro/*`
  ...
  without a server.
- **The real blocker: Stockfish.** `games/chess/stockfish.js` is a *Node*
