...
//
//   node games/chess/move-quality.mjs --arm alpha            # α=1 vs α=0
//   node games/chess/move-quality.mjs --arm null             # THE CONTROL. run it.
//   node games/chess/move-quality.mjs --arm prior            # fitted π vs uniform π
//   node games/chess/move-quality.mjs --alphas 0.5,0 --games 8 --ref-depth 14
//
...
async function replayArm(sess, seat, spec, seed) {
  ...
  let state = ChessGame.createInitialState(players, { ...sess.params.config, aiTimeMs: null, difficulty: dial });
  const picks = [];
  const log = sess.log ?? [];
  for (let i = 0; i < Math.min(log.length, maxPlies); i++) {
    ...
    if (!pa?.action) break;
    if (pa.playerId === seat) {
      const obs = ChessGame.getVisibleState(state, seat);
      ...
      const legal = ChessGame.getLegalActions({ ...state, activePlayers: [seat] }, seat);
      const tm = Date.now();
...
async function referenceAt(state, seat) {
  ...
  const childStates = legal.map(a => ChessGame.applyActions(state, [{ playerId: seat, action: a }]));
...
async function referencesFor(sess, seat) {
  const players = JSON.parse(JSON.stringify(sess.params.players));
  let state = ChessGame.createInitialState(players, sess.params.config);
  const refs = new Map();
  const log = sess.log ?? [];
  for (let i = 0; i < Math.min(log.length, maxPlies); i++) {
    const pa = log[i].playerActions?.[0];
