...
for (let p = 0; p < pairs; p++) {
  for (const priorSeat of ['white', 'black']) {
    ...
    // caught this too.
    const players = [
      { id: 'white', name: 'White', agent: new ChessObscuroAgent() },
      { id: 'black', name: 'Black', agent: new ChessObscuroAgent() },
    ];
    const engine = new GameEngine(ChessGame, players, {
      maxTurns, fogOfWar: true, fog: true, difficulty: 'easy',
    });
    const t0 = Date.now();
    const { result } = await engine.run();
    const s = Math.round((Date.now() - t0) / 1000);

    // NB: the field is `winnerId`, not `winner` (ChessGame.getResult). Reading the
