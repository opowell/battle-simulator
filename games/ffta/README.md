...
Each job has four sprites — `images/{job}/sprite_{NE,SE,SW,NW}.png` — sliced from that job's real FFTA battle sheet on spriters-resource.com (per-race frame layout: Human/Viera/Moogle/Nu Mou share one template, Bangaa another, since their side-profile frame sits in a different spot on the sheet). The suffixes are the **isometric screen** compass points (north = top of screen), not grid axes: since the board is drawn rotated 45°, each grid axis heads toward a screen diagonal (grid −y → NE, +x → SE, +y → SW, −x → NW). `toGrid` picks the file matching the unit's current `facing`, so `ui.showFacing` is off for this game — the sprite itself shows the direction instead of a facing arrow overlay.
