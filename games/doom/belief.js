// ---------------------------------------------------------------------------
// Fog-of-war belief tracking for Doom — a stateful information-set tracker,
// sibling of chess/cs/tactical/xcom belief.js. Team-based (marine / demon), so
// it is driven through DoomGame's withTeam adapter and keyed per team.
//
// Under fog getVisibleState reveals only our own team plus enemies within
...
export class DoomBelief {
  constructor(myTeam, enemyRoster) {
    this.myTeam = myTeam;
  ...
  _expand() {
    for (const pc of this.pieces.values()) {
      ...
      for (const key of pc.possible) {
        ...
        for (const t of getReachable({ x, y }, pc.reach, [])) next.add(k(t.x, t.y));
      }
  ...
  _collapse(observation) {
    ...
    for (const u of observation.units) {
      if (u.ownerId === this.myTeam) continue;
    ...
    for (const [id, pc] of this.pieces) {
      ...
      if (pc.possible.size === 0) {
        const reach = getReachable(pc.anchor, pc.reach, [])
  ...
  sample(observation, n, rng, makeUnit) {
    const myUnits = observation.units.filter(u => u.alive && u.ownerId === this.myTeam);
    const seenIds = new Set(observation.units.filter(u => u.ownerId !== this.myTeam).map(u => u.id));
...
// ---------------------------------------------------------------------------
// Per-game belief store, keyed by the (stable) players array and team.
// ---------------------------------------------------------------------------

const beliefStore = new WeakMap();

export function getDoomBelief(state, myTeam) {
  ...
  if (!belief) {
    const roster = (state.gameSpecific.startRoster ?? []).filter(u => u.ownerId !== myTeam);
    belief = new DoomBelief(myTeam, roster);
    byTeam.set(myTeam, belief);
  }
  return belief;
}
